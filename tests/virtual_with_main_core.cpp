...
struct registry_event_global_data {
    ...
    struct pw_core *core; // FIXME: remove
};
...
class VirtualAppNodesManager {
  private:
    ...
    static void ph_create_virtual_node(const post_hook_args &args) {
        ...
        struct pw_stream *virtual_stream =
            pw_stream_new(args.core, args.onode_info_ptr->media_name.c_str(), stream_props);
  ...
  public:
    // FIXME: remove core
    static void process_new_node(pw_registry *reg, pw_core *core, pw_loop *loop, uint32_t id, const char *type) {
    ...
    if (strcmp(type, PW_TYPE_INTERFACE_Node) == 0) {
        for (uint32_t i = 0; i < props->n_items; i++) {
            ...
            if (strcmp(props->items[i].value, "Chromium") == 0 ||
                ...
                VirtualAppNodesManager::process_new_node(reg_data->reg, reg_data->core,
                                                         pw_main_loop_get_loop(reg_data->main_loop), id, type);
            }
...
int main() {
    ...
    struct registry_event_global_data reg_data = {
        .main_loop = loop,
        .reg = registry,
        .core = core, // FIXME: remove
    };
